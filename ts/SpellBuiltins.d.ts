...
declare module SearchBuiltins {
  ...
  interface  {
    ...
    isCorrect(uri: xs:string, word: xs:string): xs:boolean;

    /** Suggests a list of spellings for a word. Returns a sequence of the most likely spellings for the specified word. **/
    suggest(uri: xs:string, word: xs:string, options?: (element()|map:map)): string;
    ...
    levenshteinDistance(str1: xs:string, str2: xs:string): xs:integer;
    ...
    doubleMetaphone(word: xs:string): xs:string;
    ...
    suggestDetailed(dictionary_uris: xs:string, word: xs:string, options?: (element()|map:map)): Object;
    ...
    romanize(string: xs:string): xs:string;
