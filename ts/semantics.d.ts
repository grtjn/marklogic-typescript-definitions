...
interface semFunctions {
  ...
  rdfParse(input: string, options?: string): sem.Triple;

    /** This function returns a string or json or XML serialization of the provided triples. **/
  rdfSerialize(triples: sem.Triple, options?: string): string;

    /** This function returns sem:triples from a specified location. **/
  rdfGet(location: string, options?: string, httpOpts?: MLNode<any>): sem.Triple;
  ...
  rdfInsert(triples: sem.Triple|ValueIterator<any>|Array<any>, options?: string, permissions?: Object[], collections?: string, quality?: number, forestIds?: string): string;
  ...
  rdfLoad(location: string, options?: string, httpOpts?: MLNode<any>, permissions?: string, collections?: string, quality?: number, forestIds?: string): string;
  ...
  databaseNodes(triples: sem.Triple, options?: string, query?: cts.Query): MLNode<any>;
  ...
  graphInsert(graphname: sem.Iri, triples: sem.Triple, permissions?: string, collections?: string, quality?: number, forestIds?: string): string;
  ...
  rdfBuilder(prefixes?: {[key:string]:any}, graph?: sem.Iri): (p1:string,p2:string,p3:string)=>sem.Triple;
  ...
  transitiveClosure(seeds: sem.Iri, predicates: sem.Iri, limit: number): sem.Iri;

    /** This function executes a SPARQL SELECT query using passed-in bindings participating as a starting point for the query. **/
  sparqlValues(sparql: string, values: {[key:string]:any}, options?: string, store?: string): {[key:string]:any};
