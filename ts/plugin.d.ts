...
declare module PluginModule {
  ...
  interface plugin {
    ...
    plugins(capability: xs:string, scope?: xs:string): xs:string;
    ...
    enumerate(uri: xs:string, scope?: xs:string): map:map;
    ...
    capability(capability: xs:string, uri: xs:string, scope?: xs:string): xdmp:function;
    ...
    implementations(capability: xs:string, scope?: xs:string): map:map;

    /** This function registers the capabilities map for use by MarkLogic Server. It returns the URI of the plugin. **/
    register(capabilities: map:map, pluginUri: xs:string): xs:string;
    ...
    initializeScope(scope: xs:string): xs:integer;
    ...
    flushScope(scope: xs:string, pid?: xs:string): void;
    ...
    installFromZip(scope: xs:string, zip: binary()): xs:integer;

    /** This function removes a named plugin or set of plugins from the extensions database. **/
    uninstall(scope: xs:string, pid: xs:string): void;
    ...
    asset(pid: xs:string, scope: xs:string, assetPath: xs:string): node();

    /** Returns true if the named asset exists, otherwise returns false. **/
    assetExists(pid: xs:string, scope: xs:string, assetPath: xs:string): xs:boolean;
    ...
    installFromFilesystem(scope: xs:string): xs:integer;
